...
namespace UnityThreadingExtended
{
    public abstract class DispatcherBase : IDisposable
    {
		...
		internal void AddTask(TaskBase task)
        {
            lock (taskList)
            {
                ...
                if (TaskSortingSystem == UnityThreadingExtended.TaskSortingSystem.ReorderWhenAdded ||
                    TaskSortingSystem == UnityThreadingExtended.TaskSortingSystem.ReorderWhenExecuted)
                    ReorderTasks();
            }
			dataEvent.Set();
        }
    ...
    public class Dispatcher : DispatcherBase
    {
        ...
        {
            //Debug.Log("DEBUG 1");
            if (UnityThreadHelperExtended.IsWebPlayer ? UnityThreadHelperExtended.WaitOne(dataEvent, 0) : dataEvent.WaitOne(0, false))
